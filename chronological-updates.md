...
1. Use the WSL endpoint on the ganache UI
...
On the Ganache UI, you can select a different hostname and connect to whatever IP you see. In the example below, you'd connect to `172.29.0.1`
...
3. Now replace your RPC Server URL inside the code with this latest URL that will be something like: http://172.29.0.1:7545. Recheck this by running this command:

   `grep -m 1 nameserver /etc/resolv.conf | awk '{print $2}'`.

**IF it gets stuck** at `"Deploying, please wait..."`, then you have to allow the connection for Ganache in `Windows Defender Firewall with Advanced Security`.

![img](./img/ganache-firewall.png)

1. Select Inbound Rules.
2. Select Ganache, right click and go to Properties.
3. Allow the connection and Apply.
4. Repeat for the rest, if any.
